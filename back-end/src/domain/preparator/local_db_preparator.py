...
from infra.embedder.ollama import check_ollama_model_exists
from infra.embedder.ollama import OLLAMA_MODEL_NOT_AVAILABLE_CODE
from infra.embedder.ollama import OLLAMA_SERVER_NOT_FOUND_ERROR_CODE
from infra.embedder.ollama import OLLAMA_UNKNOWN_ERROR_CODE
from infra.embedder.ollama import OllamaProcessor
...
class LocalDBPreparator():
    ...
    def __download_repo(self, repo_url: str, local_path: str, access_token: Optional[str] = None) -> str:
        ...
        try:
            ...
            access_status = self.__check_repo_accessibility(repo_url, access_token)
            ...
            if self.__is_auth_error(error_msg):
                if access_token:
                    raise HTTPException(
                        status_code=403,
            ...
            if self.__is_not_found_error(error_msg):
                raise HTTPException(
                    status_code=404,
    ...
    def __prepare_data_pipeline(self):
        """
        ...
        embedder_transformer = OllamaProcessor(embedder=embedder)
    ...
    def __transform_documents_and_save_to_db(
    ...
    def __check_repo_accessibility(self, repo_url: str, access_token: Optional[str] = None) -> int:
        """
    ...
    def __is_auth_error(self, error_msg: str) -> bool:
    ...
    def __is_not_found_error(self, error_msg: str) -> bool:
    ...
    def __download_repo_into_db(self, repo_url: str, access_token: Optional[str] = None) -> None:
        ...
        try:
            ...
            if os.path.exists(repo_dir) and os.listdir(repo_dir):
                ...
                self.__download_repo(repo_url, repo_dir, access_token)
